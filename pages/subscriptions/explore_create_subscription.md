...
| registeredWithGP = [true/false]  | 0..1 | 0..1 | This is an optional element that allows subscribers to specify if they want to receive data for the patients depending upon the status of their GP registration.<br/><br/>If the patient has a GP registration on PDS and the subscription criteria component specifies a value of `true`, then the criteria component will be considered a match.<br/><br/> If the patient does not have a registered GP on PDS and the subscription criteria component specifies a value of `false`, then the criteria component will be considered a match.<br/><br/> If the patient has a GP registration on PDS and the subscription criteria component specifies a value of `false`, then the criteria component will be considered not a match.<br/><br/>If the patient does not have a registered GP on PDS and the subscription criteria component specifies a value of `true`, then the criteria component will be considered not a match.
